...
function TambahPaslon() {
    ...
    const handleChange = (event) => {
        ...
        setDataInput((data) => {return {...data, [inputName]: inputValue}})
    }
        ...
        <div className={`flex flex-col w-full items-center pt-10 gap-5`}>
            {picture && (
                <div className='fixed w-full h-screen top-0 -z-10'>
                    ...
                    <img className="-z-20 absolute top-0 w-full h-screen object-cover" src={URL.createObjectURL(picture)} />
                </div>
            )}
            <div className='flex flex-col items-stretch justify-start w-full px-4 md:px-10'>
                ...
                <svg onClick={closeWindow} className={`${picture ? "fill-white" : "fill-black"} w-8 h-8 md:w-auto md:h-auto self-end absolute cursor-pointer`} xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40" height="40" viewBox="0 0 50 50">
            ...
            <form className='flex flex-col gap-3 w-full md:w-[40rem] mb-10'>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    ...
                    <InputText handleBlur={setDataInput} name={"no_paslon"} type={"number"} area={false} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Nama Paslon</label>
                    <InputText handleBlur={setDataInput} name={"nama_paslon"} type={"text"} area={false} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Calon Ketua</label>
                    <InputText handleBlur={setDataInput} name={"calon_ketua"} type={"text"} area={false} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Calon Wakil</label>
                    <InputText handleBlur={setDataInput} name={"calon_wakil"} type={"text"} area={false} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Visi</label>
                    <InputText handleBlur={setDataInput} name={"visi"} type={"text"} area={true} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Misi</label>
                    <InputText handleBlur={setDataInput} name={"misi"} type={"text"} area={true} />
                </div>
                <div className='bg-white/70 flex flex-col shadow-[3px_3px_3px_lightgray] p-3 rounded-xl'>
                    <label>Proker</label>
