...
const Misi = ({ id }) => {
  ...
  const paslonMisi = context.paslon[id].misi;
  const misi = paslonMisi.split("_");
  //   console.log();
  return (
    <div>
      <div>
        <ol className="list-decimal pl-5">
          {misi.map((items, index) => (
            <li>{items}</li>
          ))}
        </ol>
      </div>
...
const Proker = ({ id }) => {
  const context = useOutletContext();
  const paslonProker = context.paslon[id].proker;
  const proker = paslonProker.split("_");
  //   console.log();
  return (
    <div>
      <div>
        <ol className="list-decimal pl-5">
          {proker.map((items, index) => (
            <li>{items}</li>
          ))}
        </ol>
      </div>
...
export default function Paslon() {
  ...
  useEffect(() => {
    try {
      const ApiDataPaslon = context.paslon;
      const ApiDataLogin = context.dataLogin;
      if (ApiDataPaslon && ApiDataLogin) {
        setPaslon(ApiDataPaslon);
        setIsVote(!ApiDataLogin.status);
        setLoading(false);
        if (ApiDataLogin.hak == "admin") {
          setIsAdminPage(true);
        } else {
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          {isVote & context.pilih & !isAdminPage ? (
          ...
          <div className="text-white flex flex-wrap items-stretch justify-center gap-10 w-full h-screen py-10">
                ...
                <div className="modal" data-theme="cupcake">
                  <div className="modal-box text-black">
                    <div className="isi">
                      ...
                      <div className="misi text-sm mt-5">
                        <Misi id={index} />
                      </div>
                      <h1 className="font-bold text-2xl mt-5">PROKER</h1>
                      <div className="proker text-sm mt-5">
                        <Proker id={index} />
                      </div>
                    </div>
            ...
            {isAdminPage && (
              <div
                onClick={() => navigate("/admin/tambahpaslon")}
                className="overflow-hidden shadow-[1px_1px_8px_black] rounded-xl bg-gradient-to-br from-blue-600 from-10% to-[#40128B] flex flex-col justify-center items-center cursor-pointer w-60 h-[25.5rem] relative"
              >
